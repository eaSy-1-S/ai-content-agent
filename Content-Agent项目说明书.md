...
- 业务流程拆解：把公众号内容生产拆成选题、生成、审核、同步、发布多个环节。
- AI 工作流设计：不是单点调用模型，而是设计输入、生成、审核、优化、输出的完整链路。
- 产品取舍：保留人工审核发布，避免为了自动化牺牲内容安全。
- 数据结构意识：用飞书表格字段承载状态流转，让 AI 结果可管理、可追踪。
- 落地能力：不仅有想法，还完成了飞书、Dify、微信 API 的集成闭环。
